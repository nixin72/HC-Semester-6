    ...
    public class RACRequestBLL
    {
        ...
        public static RACRequest ChangeProgram(Candidate candidate, int programId, bool IsGenEdOnly)
        {
                ...
                foreach (Competency comp in IsGenEdOnly == false
                    ? rac.Program.Competencies.Where(x=> x.IsActive)
                    : rac.Program.Competencies.Where(x => x.CompetencyMinistryData.IsGenEd && x.IsActive))
                {
                    ...
                    candidate.CompetencyComments.Add(competencyComment);
