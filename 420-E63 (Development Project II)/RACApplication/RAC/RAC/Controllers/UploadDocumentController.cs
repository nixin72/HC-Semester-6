...
namespace RAC.Controllers
{
    public class UploadDocumentController : Controller
    {
        ...
        private const string JSON_EXCEPTION_NULL = "{ \"exception\": \"NullReferenceException\"}";
        ...
        public JsonResult SaveChangesToDb()
        {
            ...
            RACRequest currentRac = can.CandidateProfile.RACRequest;
            DocumentList.Instance.SaveChangesToDb(currentRac);

            if (RACAdvisorBLL.CheckIsRACAdvisor())
            {
                RACAdvisorBLL.SendUploadNotification(can.CandidateProfile.Id);

                var url = Url.Action("Login", "Account", null, protocol: Request.Url.Scheme);
                Notification.sendEmailWithMessage(can.Email, "Hello " + can.FirstName + "<br />A new document has been uploaded to you by the RAC Advisor. To view this document, <a href='"+url+"'>sign in here.</a>", "New Documents");
            }
        ...
        public JsonResult IsTotalFileSizeValid(bool isAddedByRACAdvisor)
        {
            ...
            {
                RACUser can = CandidateBLL.GetCandidateByUserType();
                int racRequestId = can.CandidateProfile.RACRequest.Id;
                List<UploadedDocument> uploadedDocuments =
                    UploadedDocumentBll.GetUploadedDocuments(racRequestId, isAddedByRACAdvisor);
                ...
                return Json(totalCombinedDocumentSize >
                            Convert.ToInt32(WebConfigurationManager.AppSettings.Get("MaxTotalFileSize"))
                    ? JSON_BOOLEAN_FALSE
                    : JSON_BOOLEAN_TRUE);
            }
            catch (NullReferenceException)
            {
                return Json(JSON_EXCEPTION_NULL);
            }
        }
        ...
        public ActionResult SaveUploadedFileLocal(bool isAddedByRACAdvisor)
        {
            ...
            {
                RACUser can = CandidateBLL.GetCandidateByUserType();
                RACRequest currRac = can.CandidateProfile.RACRequest;
                foreach (string fileName in Request.Files)
                {
                    HttpPostedFileBase file = Request.Files[fileName];
                    if (file != null)
                    {
                        DocumentList.Instance.SaveDocumentsLocal(file.FileName, file.InputStream, file.ContentLength,
                            currRac, isAddedByRACAdvisor);
                    }
                    else
                    {
                        return Json(new {Message = "File is null"});
                    }
                }
