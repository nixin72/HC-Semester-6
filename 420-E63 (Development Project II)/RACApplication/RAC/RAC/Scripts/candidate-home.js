$(document).ready(function() {
    ...
    for (let idx in elementAnswers) {
        if (elementAnswers.hasOwnProperty(idx)) {
            $(elementAnswers[idx]).click(function() {
                checkForCommentsRequired();
                saveRACRequest();
            });
        }
    ...
    for (let idx in selectAllButtons) {
        if (selectAllButtons.hasOwnProperty(idx)) {
            $(selectAllButtons[idx]).click(function() {
                selectAllAnswers();
            });
        }
    }

    const commentBoxes = document.querySelectorAll('#competencyCommentInstance_Comment');
    commentBoxes.forEach(function(commentBox) {
        $(commentBox).blur(saveRACRequest());
    });
