...
-- Description:	Select usernames from the Users table in CSAdmin
-- Revision History:
...
-- =============================================
CREATE PROCEDURE [dbo].[uspSelectUsernames]
...
BEGIN
	...
	*/
	...
	SELECT u.LastName, u.Username, u.IsActive, u.FirstName, u.IDUser, ad.ADUsername
		FROM Users.[User] u
		...
		WHERE ((u.FirstName COLLATE SQL_Latin1_General_CP1_CI_AI like '%' + @SearchFirstName + '%' COLLATE SQL_Latin1_General_CP1_CI_AI)
		...
		ORDER BY
		CASE
			WHEN @SortExpression is NULL THEN u.LastName + u.FirstName
		END ASC,
		CASE
			WHEN @SortExpression = 'Name' AND @SortDirection = 0 THEN u.LastName + u.FirstName
			WHEN @SortExpression = 'Username' AND @SortDirection = 0 THEN u.Username
		END ASC,
		CASE
			WHEN @SortExpression = 'Name' AND @SortDirection = 1 THEN u.LastName + u.FirstName
...
END
GO
GRANT EXECUTE ON  [dbo].[uspSelectUsernames] TO [CSAdmin]
GO
